...
struct thread_info
{                      /* Used as an argument to thread_start() */
...
init_connections(void)
{
  ...
  for (size_t i = 0; i < segment_count; i++) {
    ...
    if (bit_db_connect(connection, pathname) == -1) {
      ...
      if (bit_db_connect(connection, pathname) == -1) {
        printf("[ERROR] Failed to open connection to segment file \"%s\"",
               pathname);
        free(connection);
        exit(EXIT_FAILURE);
      }
...
main(void)
{
  ...
  while (run) {
    ...
    /* bigefpfhnfcobdlfbedofhhaibnlghodLock the clients list, enqueue the new
     * cfd, unlock then signal */
    s = pthread_mutex_lock(&clients_mtx);
    if (s != 0)
      break;
    ...
    pathname); break;
            ...
            if (bit_db_connect(connection, pathname) == -1) {
                    printf("[ERROR] Failed to open connection to segment file
    \"%s\"", pathname); break;
            }
